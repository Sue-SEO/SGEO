...
import { fetchKeywords, saveKeywords } from "@/lib/db";
import type { KeywordUiRow, KeywordCategory, ClassifyResultItem } from "@/lib/types";
...
const CATS: Record<KeywordCategory, CategoryMeta> = {
...
const TABS: { key: "all" | KeywordCategory; label: string }[] = [
  { key: "all", label: "全部" },
...
function parseInput(text: string): KeywordUiRow[] {
...
async function classifyBatch(
  ...
  try {
    ...
    return data.results as ClassifyResultItem[];
  } catch {
...
export default function KeywordSorter() {
  const [rows, setRows] = useState<KeywordUiRow[]>([]);
  ...
  function handlePasteLoad(text: string) {
  ...
  const startSort = useCallback(async () => {
    ...
    let working: KeywordUiRow[] = rows.map((r) => {
      if (r.category !== "pending") return r;
      if (brands.some((b) => b && r.keyword.includes(b))) {
        return { ...r, category: "competitor" as KeywordCategory, reason: "命中竞品品牌词" };
      }
      if (negs.some((n) => n && r.keyword.includes(n))) {
        return { ...r, category: "negative" as KeywordCategory, reason: "命中排除规则词" };
      }
      return r;
    });
    setRows(working);
  ...
  function overrideCategory(id: string, newCat: KeywordCategory) {
    setRows((prev) =>
  ...
  async function handleSaveToDb() {
    ...
    setSaving(true);
    setSaveMsg("");
    const result = await saveKeywords(rows, seedTopic);
    if (result.ok) {
      setSaveMsg(`已保存 ${result.count} 条到数据库`);
    } else {
  ...
  function exportCSV(which: "current" | "relevant") {
    const target =
      which === "current" ? filteredRows : rows.filter((r) => r.category === "relevant");
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "24px" }}>
        ...
        {saveMsg && (
          <p
            style={{
              fontSize: "12px",
              color: saveMsg.startsWith("保存失败") ? "#B45309" : "#1F8A5A",
              marginTop: "-12px",
            }}
          >
            {saveMsg}
        ...
        <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
          ...
          >
            ...
            >
              {processing
                ? `分拣中 ${progress.done}/${progress.total}`
                : `开始分拣（共 ${rows.length} 条）`}
            </button>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "16px" }}>
              {(["all", "relevant", "competitor", "negative", "irrelevant", "uncertain"] as const).map(
                (key) => {
                  const isAll = key === "all";
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button
                  onClick={() => exportCSV("relevant")}
                  style={{
                    fontSize: "13px",
                    padding: "6px 12px",
                    borderRadius: "6px",
                    border: "1px solid #146B69",
                    background: "#146B69",
                    color: "#fff",
                  }}
                >
                  ...
                  <table style={{ width: "100%", fontSize: "14px", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #E2E4E9" }}>
                        <th style={{ textAlign: "left", padding: "8px 8px 8px 12px", color: "#6B7280", fontWeight: 500 }}>
                          关键词
                        </th>
                        <th style={{ textAlign: "right", padding: "8px", color: "#6B7280", fontWeight: 500, width: "90px" }}>
                          搜索量
                        </th>
                        <th style={{ textAlign: "right", padding: "8px", color: "#6B7280", fontWeight: 500, width: "70px" }}>
                          KD
                        </th>
                        <th style={{ textAlign: "left", padding: "8px", color: "#6B7280", fontWeight: 500, width: "100px" }}>
                          分类
                        </th>
                        <th style={{ textAlign: "left", padding: "8px", color: "#6B7280", fontWeight: 500 }}>原因</th>
                        <th style={{ textAlign: "left", padding: "8px 12px 8px 8px", color: "#6B7280", fontWeight: 500, width: "120px" }}>
                          调整
                        </th>
                      </tr>
                    </thead>
                          ...
                          <tr key={r.id} style={{ borderBottom: "1px solid #F0F1F3" }}>
                            ...
                            <td style={{ padding: "8px 12px 8px 8px" }}>
                              <select
                                value={r.category}
                                onChange={(e) =>
                                  overrideCategory(r.id, e.target.value as KeywordCategory)
                                }
                                style={{
                                  fontSize: "12px",
                                  borderRadius: "4px",
                              ...
                              >
                                {(Object.keys(CATS) as KeywordCategory[])
                                  .filter((k) => k !== "pending")
                                  .map((k) => (
