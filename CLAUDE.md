...
1. 在 `lib/types.ts` 加对应的类型（比如 `GscPerformanceRow`）
2. 在 `lib/db.ts` 加对应的读写函数（比如 `fetchGscPerformance()`）
3. 如果需要调用外部API且涉及密钥，在 `app/api/<name>/route.ts` 写服务器端接口
4. 在 `app/<module>/` 下建页面和组件，组件里只调用 `lib/db.ts` 的函数，不直接碰 Supabase
5. 在 `app/page.tsx` 的模块列表里加一张卡片
